...
figure(1), clf
subplot(231)
topoplot(squeeze(pow1(:,freqidx(1))),ALLEEG(1).chanlocs);
set(gca,'clim',[-200 200])
title('10 Hz target')

subplot(232)
topoplot(squeeze(pow2(:,freqidx(1))),ALLEEG(1).chanlocs);
set(gca,'clim',[-80 80])
title('10 Hz flankers')

subplot(233)
topoplot(squeeze((mean(pow1(:,freqidx),2) + mean(pow2(:,freqidx),2))./2),ALLEEG(1).chanlocs);
set(gca,'clim',[-200 200])
title('Average')
...
for condi=1:2
    ...
    subplot(2,3,condi)
    tmpmap = pinv(evecs');
    topoplot(tmpmap(:,end)./max(abs(tmpmap(:,end))),ALLEEG(1).chanlocs);
    set(gca,'clim',[-1 1])
...
set(gca,'xlim',[1 51])
title('Static SSVEP from components')
...
% Goal: find a spatial filter that best differentiates activity between correct
...
subplot(121)
topoplot(maps(:,eigidx(end)),EEG.chanlocs,'numcontour',0);
...
filter_dataE = abs(hilbert( reshape(EEG.data,EEG.nbchan,[])' * evecs(:,eigidx(end)) )).^2;
...
set(gca,'xlim',[-300 1200]), axis square
legend({'Correct response';'Error response'})

%% GED for error-correct, part II (this time without bias)

% load in data again (because EEG.data are filtered)
load CEdata.mat

% compute covariance of broadband data
datBB = reshape(EEG.data(:,tidx(1):tidx(2),:),EEG.nbchan,[]);
datBB = bsxfun(@minus,datBB,mean(datBB,2));
covBB = (datBB*datBB') / size(datBB,2);

% filter data
EEG.data = filterFGx(EEG.data,EEG.srate,6,3);
% extract and vectorize data, and compute covariance matrices
datfilt = reshape(EEG.data(:,tidx(1):tidx(2),:),EEG.nbchan,[]);
datfilt = bsxfun(@minus,datfilt,mean(datfilt,2));
covfilt = (datfilt*datfilt') / size(datfilt,2);

% GED
[evecs,evals] = eig(covfilt,covBB);
