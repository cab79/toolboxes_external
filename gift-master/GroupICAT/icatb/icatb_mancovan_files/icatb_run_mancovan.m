        ...
        if (strcmpi(features{nF}, 'spatial maps'))
            ...
            for ncomps = comp_inds
                ...
                for nM = 1:length(avg_runs_info)
                    if (length(avg_runs_info{nM}) > 1)
                        tmpMap = squeeze(icatb_nanmean(reshape(SM(avg_runs_info{nM}, :), [length(avg_runs_info{nM}), 1, size(SM, 2)]), 1));
                    else
                        tmpMap = SM(avg_runs_info{nM}, :);
                    end
                    SM_avg(nM, :) = tmpMap(:)';
                end
                ...
                if (~strcmpi(feature_params.final.sm_mask, 'default'))
                    if (countComp == 1)
                        ...
                        tmpSubCount = zeros(Vol.dim(1:3));
                        tmpSubCount(mask_ind) = sum(~isnan(SM));
                        
                        
                        tmpSubCountName = fullfile(dirName, [mancovanInfo.prefix, '_sub_count.nii']);
                        Vol.fname = fullfile(mancovanInfo.outputDir, tmpSubCountName);
                        Vol.n(1) = 1;
                        
                        icatb_write_vol(Vol, tmpSubCount);
...
if (strcmpi(mancovanInfo.designCriteria, 'mancova'))
    ...
    if (isempty(univInfo))
        ...
        if length(test_names) > 0
            ...
            for ii = 2:length(sig_terms)  % no test for constant
                ...
                [ t_u{ii-1}, p_u{ii-1}, stats_u{ii-1}] = icatb_nan_mT(data, X_reduced, stats_m.Terms, sig_terms{ii}, {'verbose'});
                addTag = 1;
        ...
        for nUnivInfo = 1:length(univInfo)
            ...
            [ t_u{nUnivInfo}, p_u{nUnivInfo}, stats_u{nUnivInfo}] = icatb_nan_mT(data, X_reduced, allTerms, termNo, {'verbose'});
            addTag = 1;
    ...
    if (strcmpi(mancovanInfo.designCriteria, 'one sample t-test'))
        [ t_u{1}, p_u{1}, stats_u{1}] = icatb_nan_mT(data, mancovanInfo.X, [], 0, {'verbose'});
    ...
    elseif (strcmpi(mancovanInfo.designCriteria, 'two sample t-test'))
        [t_u{1}, p_u{1}, stats_u{1}] = icatb_nan_mT(data, mancovanInfo.X, [], 1, {'verbose'});
        ...
        [t_u{1}, p_u{1}, stats_u{1}] = icatb_nan_mT(data(1:N1, :) - data(N1 + 1:end, :), mancovanInfo.X, [], 0, {'verbose'});
        tmp_con_name = [mancovanInfo.ttestOpts.t.name{1}, ' - ', mancovanInfo.ttestOpts.t.name{2}];
        stats_u{1} = get_contrast_label(stats_u{1}, ttestNames, ttestNames, {tmp_con_name});
    end
