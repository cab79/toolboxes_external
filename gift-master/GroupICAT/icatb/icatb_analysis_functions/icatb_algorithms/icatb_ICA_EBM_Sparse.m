function [W,totalIterSparse,Cost,independence,sparsity] = icatb_ICA_EBM_Sparse(X, varargin) %lambda,epsilon)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SparseICA-EBM: Sparse ICA by Entropy Bound Minimization (real-valued version)
% Four nonlinearities
% x^4,  |x|/(1+|x|),    x|x|/(10+|x|),  and     x/(1+x^2)
% are used for entropy bound calculation and the l^1 regularization term is
% used to promote sparsity.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% INPUT:
% X: mixtures (Dimesion NxT, where T is the number of samples)
% lambda: Sparsity parameter 
% epsilon: Smoothing parameter 
% OUTPUT:
% W: demixing matrix (Dimension NxN)
% totalIterSparse: Number of iterations for the SparseICA-EBM non-orthogonal stage
% Cost: Cost function 
% independence: Independence term in the cost function. It is described by
% the negative of the ICA maximum likelihood function 
% sparsity: Sparsity term in the cost function. It is described by the
% product of the l^1 regularization term and the parameter lambda

% Program SparseICA-EBM by Zois Boukouvalas (zb1@umbc.edu). V1-04/28/2017
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% References:
%
% [1] Z. Boukouvalas, Y. Levin-Schwartz, Vince D. Calhounc, and T. Adali, "Sparsity and Independence: Balancing of two Objectives in Optimization for Source Separation with Application to fMRI Analysis," 
% Elsevier, Journal of the Franklin Institute (JFI), Engineering and Applied Mathematics, 2017. 

% [2] Z. Boukouvalas, Y. Levin-Schwartz, and T. Adali, "Enhancing ICA performance by exploiting sparsity: Application to fMRI Analysis,"
% in Proc. IEEE Int. Conf. Acoust., Speech, Signal Processing (ICASSP), New Orleans, LA, March 2017 
...
for n = 1:2:length(varargin)
   if (strcmpi(varargin{n}, 'lambda'))
       lambda = varargin{n + 1};
   elseif (strcmpi(varargin{n}, 'epsilon'))
...
% These is the norm 1 function and it derivative
rG2.func=@(x) sqrt(x.^2 + epsilon);
rG2.diff=@(x) x.*(1./sqrt(x.^2 + epsilon));
...
% [End of Part 0]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%           Part I: orthogonal SparseICA
%  varying step size, stochastic gradient search 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if verbose
    fprintf('\nOrthogonal SparseICA stage.');
end
last_W = W;
best_W = W;
...
for iter = 1 : max_iter_orth
    ...
    Y = W*X;
    for kk = 1:N
        norms(kk,1) = norm(Y(kk,:),1);
    end
    
    Cost(iter) = (lambda)*(sum(norms)); %Impose Sparsity to the total Cost function
    ...
    for n = 1 : N
        ...
        if fastica_on
            w1 = grad - Edgx*w - (lambda)*X*rG2.diff(y)'; %Impose sparsity
            
        else
            grad = vEGx*grad - (lambda)*X*rG2.diff(y)'; %Impose sparsity
            grad = grad - (w'*grad)*w;
            grad = grad / norm(grad);
            w1 = w + mu*grad;
        end
...
if SADDLE_TESTED == 1
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %           Part III: if saddles are detected, refine orthogonal SparseICA
    %  fixed step size, gradient search
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if verbose
        fprintf('\nOrthogonal SparseICA refinement is required because saddles are detected.');
    end
    last_W = W;
    best_W = W;
    ...
    for iter = 1 : max_iter_orth_refine
        ...
        Cost(iter) = (lambda)*(sum(norms)); %Impose sparsity to the cost function
    ...
    if show_cost
        ...
        xlabel('Number of iterations')
        ylabel('Cost')
        title('(b) Refine orthogonal SparseICA')
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%           Part IV: non-orth SparseICA 
%   fixed small step size for refinement, gradient search
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%W = Winit;
if verbose
    fprintf('\nNonorthogonal Sparse ICA stage.');
end
%last_W = Winit/P;
last_W = W;
%best_W = Winit/P;
...
sparsity = zeros(max_iter_nonorth,1);
independence = zeros(max_iter_nonorth,1);
Cost1 = zeros(max_iter_nonorth,1);
...
for iter = 1 : max_iter_nonorth
    ...
    Cost(iter) = -log(abs(det(W))) + lambda*(sum(norms)); %Impose sparsity to the Cost function
    Cost1(iter) = -log(abs(det(W)));
    ...
    for n = 1 : N
        ...
        Cost(iter) = Cost(iter) - max_NE;
        Cost1(iter) = Cost1(iter) -  max_NE;
        sparsity(iter) = sparsity(iter) + lambda*norm(y,1); %This is the sparsity term in the cost function
        independence(iter) = Cost1(iter); %This is the independence term in the cost function
...
W = best_W;
W = W*P;
totalIterSparse = iter;
%fprintf('\n****Total iterations NonOrthogonal: %d\n', iter);
if show_cost
    subplot(1,4,4);
    plot([1:iter],Cost([1:iter]));
    xlabel('Number of iterations')
    ylabel('Cost')
    title('(c) Nonorthogonal SparseICA')
end
