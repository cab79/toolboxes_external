...
try
    if isfield(sesInfo.userInput.HInfo.V, 'gifti')
        [ddd, gfn] = fileparts(sesInfo.userInput.files(1).name(1,:));
        sesInfo.icaOutputFiles = icatb_getOutputFileNames([sesInfo.userInput.prefix, '_', gfn], sesInfo.numComp, sesInfo.numOfSub, sesInfo.numOfSess, sesInfo.write_analysis_steps_in_dirs, '.gii');
    else
