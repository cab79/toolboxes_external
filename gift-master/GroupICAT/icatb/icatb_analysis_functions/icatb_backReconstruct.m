...
if (~useTemporalICA)
    ...
    if (isempty(icatb_findstr(lower(algorithmName),'iva')))
        ...
        if (~strcmpi(backReconType, 'spatial-temporal regression') && ~strcmpi(backReconType, 'gig-ica') ...
                && ~strcmpi(backReconType, 'constrained ica (spatial)'))
...
backReconFiles = cellstr(strcat(fullfile(outputDir, [sesInfo.back_reconstruction_mat_file]), icatb_numberToString(dataSetsToRun(:)), '.mat'));
