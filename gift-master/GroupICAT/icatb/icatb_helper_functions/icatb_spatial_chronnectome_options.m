function input_parameters = icatb_spatial_chronnectome_options(varargin)
...
inputParameters(numParameters).listString = 'Dynamic Coupling defs';
optionNumber = 1;
% Option 1 of parameter 2
options(optionNumber).promptString = 'Select connectivity Method';
options(optionNumber).answerString = char('Correlation', 'Partial Correlation', 'Mutual Information');
options(optionNumber).uiType = 'popup'; options(optionNumber).value = 1;
