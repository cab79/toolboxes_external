...
est_options = {'MDL (i.i.d Sampling)', 'MDL (FWHM)',  'Entropy Rate (Finite Memory Length)', 'Entropy Rate (AR)'};
...
try
    ...
    try
        dim_est_opts = sesInfo.userInput.dim_est_opts;
    catch
        dim_est_opts = DIM_ESTIMATION_OPTS;
    end
    
    try
        estimation_method = dim_est_opts.method;
    catch
    end
    ...
    if strcmpi(answerQuestion, 'yes') %answerQuestion == 1
        ...
        if answerQuestion2 == 1
            ...
            if (strcmpi(parallelMode, 'serial'))
                [estimateComp, estCompVec, meanMdl] = serial_computeMDL(sesInfo, handles, mask, precisionType, dim_est_opts);
            else
                if (parallelCluster)
                    try
            ...
            [estCompVec, mdlVal, aicVal] = icatb_estimate_dimension(sesInfo.userInput.files(answerQuestion3).name, mask, precisionType, dim_est_opts);
    ...
    if strcmpi(answerQuestion, 'yes') %answerQuestion == 1
        ...
        if answerQuestion2 == 0 || numOfSub*numOfSess == 1
            ...
            msgString = ['The estimated independent components is found to be ', num2str(estimateComp), ...
                ' using the ', est_options{estimation_method}, ' criteria.'];
            if ( doGetPrompts )
            ...
            string1 = ['1. The estimated independent components is found to be ', num2str(estimateComp), ...
                ' using the ', est_options{estimation_method}, ' criteria.'];
            string2 = ['2. The mean of estimated components over all subjects is found to be ', num2str(meanSub), '.'];
            string3 = ['3. The standard deviation of estimated components over all subjects is found to be ', num2str(stdSub), '.'];
            string4 = ['4. The minimum of estimated components over all subjects is found to be ', num2str(minSub), '.'];
            string5 = ['5. The maximum of estimated components over all subjects is found to be ', num2str(maxSub), '.'];
            string6 = ['6. The median of estimated components over all subjects is found to be ', num2str(median(estCompVec)), '.'];
...
function [estimateComp, estCompVec, meanMdl] = serial_computeMDL(sesInfo, handles, mask, precisionType, dim_est_opts)
...
for numFiles = 1:length(sesInfo.userInput.files)
    ...
    [ estCompVec(numFiles), mdlVal] = icatb_estimate_dimension(sesInfo.userInput.files(numFiles).name, mask, precisionType, dim_est_opts);
    %     if strcmp(flagTimePoints, 'same_time_points')
    %
