...
[pathstr, fName, extn] = fileparts(icatb_parseExtn(deblank(files(1).name(1, :))));
extn = deblank(extn);

if (~strcmpi(extn, '.nii')  && ~strcmpi(extn, '.img') && ~strcmpi(extn, '.gz') && ~strcmpi(extn, '.gii'))
    error('Check the mask format.');
end
