...
check_nuisance = icatb_questionDialog('title', 'Remove nuisance covariates', 'textbody', 'Do you want to remove variance associated with nuisance covariates?');
nuisance_cov_file = '';
drawnow;
if (check_nuisance)
    nuisance_cov_file = icatb_selectEntry('title', 'Select nuisance covariates file (continuous covariates only)', 'typeEntity', 'file', 'typeSelection', 'single', 'filter', '*.asc;*.dat');
end
drawnow;
...
figureData.nuisance_cov_file = nuisance_cov_file;
...
try
    ...
    if (~isempty(figData.nuisance_cov_file))
        
        tmp_nuisance = icatb_load_ascii_or_mat(nuisance_cov_file);
        tmp_nuisance = icatb_zscore(tmp_nuisance);
        
        num_states_clusters = size(dfnc_corrs);
        num_states_clusters = num_states_clusters(end);
        
        for nSess = 1:size(dfnc_corrs, 1)
            for nStates = 1:num_states_clusters
                tmp_dfnc_corrs = squeeze(dfnc_corrs(nSess, :, :, nStates));
                
                chkNotNan = isnan(sum(tmp_dfnc_corrs, 2))==0;
                chkNotNan = prod(chkNotNan, 2)==1;
                
                tmp_dfnc_corrs = regress_cov(tmp_dfnc_corrs(chkNotNan, :), tmp_nuisance(chkNotNan, :));
                
                dfnc_corrs(nSess, chkNotNan, :, nStates) = tmp_dfnc_corrs;
            end
        end
        
        if (exist('dfncTaskConnectivity', 'var'))
            for nSess = 1:size(dfncTaskConnectivity, 1)
                for nR = 1:size(dfncTaskConnectivity, 4)
                    dfncTaskConnectivity(nSess, :, :, nR) = regress_cov(squeeze(dfncTaskConnectivity(nSess, :, :, nR)), tmp_nuisance);
                end
            end
        end
    ...
    if (strcmpi(designCriteria, 'one sample t-test'))
        ...
        if (exist('dfncTaskConnectivity', 'var'))
            mean_task_conn = cell(1, length(selectedRegressors));
            t_task_conn = mean_task_conn;
            p_task_conn = mean_task_conn;
            stats_task_conn = mean_task_conn;
            for nRegress = 1:length(selectedRegressors)
                ...
                tmp = squeeze(dfncTaskConnectivity(:, :, nRegress));
                mean_task_conn{nRegress} = mean(tmp);
                [t_task_conn{nRegress}, p_task_conn{nRegress}, stats_task_conn{nRegress}] = mT(tmp, modelX, [], 0, {'verbose'});
            end
    ...
    elseif (strcmpi(designCriteria, 'paired t-test'))
        ...
        if (exist('dfncTaskConnectivity', 'var'))
            ...
            for nRegress = 1:length(selectedRegressors)
                disp(['Computing paired t-test on task connectivity (', selectedRegressors{nRegress}, ' ...']);
                tmp1 = squeeze(dfncTaskConnectivity(g1, :, nRegress));
                tmp2 = squeeze(dfncTaskConnectivity(g2, :, nRegress));
                mean_task_conn{1, nRegress} = mean(tmp1);
                mean_task_conn{2, nRegress} = mean(tmp2);
                [t_task_conn{nRegress}, p_task_conn{nRegress}, stats_task_conn{nRegress}] = mT(tmp1 - tmp2, modelX, [], 0, {'verbose'});
            end
            save(outFile, 't_task_conn', 'p_task_conn', 'stats_task_conn', 'mean_task_conn', '-append');
        end
