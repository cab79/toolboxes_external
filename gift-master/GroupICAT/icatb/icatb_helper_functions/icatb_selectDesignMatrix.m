...
% D(size(D, 2) + 1).string = '';
D(size(D, 2) + 1).string = 'Same Set of regressors for all subjects and sessions - One SPM model is used for all the subjects and sessions. Same set of regressors will be applied to all data-sets.';
D(size(D, 2) + 1).string = '';
D(size(D, 2) + 1).string = 'Different set of regressors for all sessions - One SPM model is used for all subjects and sessions. Session specific regressors will be pulled during temporal sorting.';
D(size(D, 2) + 1).string = '';
D(size(D, 2) + 1).string = 'Different set of regressors for all subjects and sessions - One SPM model is used for each subject''s session. Regressors pulled during temporal sorting will be specific to subjects''s session.';
...
if numOfSub*numOfSess > 1
    ...
    InputHandle = icatb_getGraphics('Selecting Design Matrix', 'normal', 'SPM Design Matrix'); % figure handle
    % set menubar none
    ...
    switch lower(answerString)
        case 'same_sub_same_sess'
            if strcmp(lower(flagTimePoints), 'same_time_points')
                ...
                [SPMFiles.name] = icatb_selectEntry('title', 'Select SPM design matrix', 'filter', '*.mat', ...
        ...
        case 'diff_sub_diff_sess'
            ...
            tempSPMFiles = icatb_selectEntry('title', 'Select SPM design matrix/matrices', 'filter', '*.mat', ...
