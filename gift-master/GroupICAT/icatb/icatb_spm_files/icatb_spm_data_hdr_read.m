function V = icatb_spm_data_hdr_read(P)
...
switch class(P)
    ...
    case 'cell'
        V = cellfun(@icatb_spm_data_hdr_read, P, 'UniformOutput',false);
        
    case 'char'
        P = cellstr(P);
        switch lower(icatb_spm_file(P{1},'ext'))
            case {'nii','hdr','img'}
                V = icatb_spm_vol(char(P));
...
V = struct(...
    'fname',   '',...
    'dim',     [0 0 0],...
    'dt',      [icatb_spm_type('float64') icatb_spm_platform('bigend')],...
    'pinfo',   [1 0 0]',...
    'mat',     eye(4),...
    'n',       [1 1],...
    'descrip', '',...
