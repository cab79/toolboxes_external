...
for i = 1:2:length(varargin)
    ...
    elseif (strcmpi(varargin{i}, 'colorbar'))
        colorbarPlot = varargin{i + 1};
         elseif (strcmpi(varargin{i}, 'coords'))
        coords = varargin{i + 1};
    end
end
...
for N = 1:length(network_names)
    ...
    for nComp = 1:length(comps)
        ...
        hD = icatb_orth_views(currentFile, 'structfile', structFile, 'image_values', image_values, 'convert_to_zscores', convert_to_z, 'labels', ...
            ...
            hD = icatb_orth_views(currentFile, 'structfile', structFile, 'image_values', image_values, 'convert_to_zscores', convert_to_z, 'labels', ...
            [prefix, ' ', num2str(comps(nComp))], 'get_interp_data', 1, 'set_to_max_voxel', 0, 'threshold', thresholds(N, :), 'coords', coords{N}(nComp, :));
        ...
        tmpStr = [prefix,  num2str(comps(nComp)), ' (', num2str(realCoords(1)), ',', num2str(realCoords(2)), ',', num2str(realCoords(3)), ')'];
        ...
        str = charC ([prefix,  num2str(comps(nComp))], ['(', num2str(realCoords(1)), ',', num2str(realCoords(2)), ',', num2str(realCoords(3)), ')']);
        if (interMediatePlot)
