...
getMask = deblank(getString(getMask, :));
...
% Do estimation
dim_est_opts = selectDimEstOptions;
handles_data = get(handles, 'userdata');
handles_data.sesInfo.userInput.dim_est_opts = dim_est_opts;
set(handles, 'userdata', handles_data);
...
function dim_est_opts = selectDimEstOptions
%% Select dimensionality estimation options

opts = {'MDL (i.i.d Sampling)', 'MDL (FWHM)',  'Entropy Rate (Finite Memory Length)', 'Entropy Rate (AR)'};

numParameters = 1;

inputText(numParameters).promptString = 'Select dimensionality estimation method';
inputText(numParameters).answerString = opts;
inputText(numParameters).uiType = 'popup';
inputText(numParameters).dataType = 'string';
inputText(numParameters).tag = 'estimation_method';
...
inputText(numParameters).promptString = 'Enter FWHM value (used in MDL (FWHM) method)';
inputText(numParameters).answerString = '[5, 5, 5]';
inputText(numParameters).uiType = 'edit';
inputText(numParameters).dataType = 'numeric';
inputText(numParameters).tag = 'est_fwhm';
inputText(numParameters).enable = 'on';
inputText(numParameters).value = 1;
inputText(numParameters).flag = 'scalar';

answer = icatb_inputDialog('inputtext', inputText, 'Title', 'Select Dimensionality Estimation Options', 'handle_visibility',  'on', 'windowStyle', 'modal');


try
    inds = strmatch(lower(answer{1}), lower(opts), 'exact');
    dim_est_opts.method = inds(1);
    
    dim_est_opts.fwhm = answer{2};
catch
