function [images, recons, w, n, mu] = decode_mixtmod_manyK(X, Y, trainidx, testidx, B, Sigma, prior_mu, IDX, prior, sel, T)
%
% Forming reconstructions from the encoding models and prior information
% with Gaussian mixture models with unlabeled data
%
% Input
...
% prior_mu: means for all components in the prior
% IDX: the component to which an image from the prior belongs
% prior: images for the prior
% sel: selection of pixels that are non-zero in the images
...
prior_sigma = zeros(pixsize,pixsize,1);
for i=1:numcat,
    disp(i)

    %calculate Sig_prior per category to limit memory usage
    prior_sigma(1:pixsize,1:pixsize,1) = cov(prior(IDX==i,sel));

    Uinv = eye(pixsize) + prior_sigma(:,:,1)*D;
    Umui = Uinv \ prior_mu(i,:)';
    Qi = Uinv \ prior_sigma(:,:,1);
    
    Qizbar = Qi*zbar;
    n(:,:,i) = bsxfun(@plus,Qizbar,Umui)';
    w(:,i) = -sum(log(diag(chol(Uinv*Uinv'))))/2 - prior_mu(i,:)*D*Umui/2 + ...
        diag(zbar'*Qizbar)/2 + zbar'*Umui;
