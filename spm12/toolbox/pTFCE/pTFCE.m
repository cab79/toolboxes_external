...
if any(imgZ<0,'all')
    error('Negative Z values - re-run after inverting!');
end
...
% make a loop instead:
PVC(PVC>1)=1; PVC(isnan(PVC))=1; % cab added
pTFCE=ones(size(imgZ));
for i=1:size(imgZ(:))
    [x,y,z]=ind2sub(size(imgZ), i);
    pTFCE(x,y,z) = exp( -aggregate_logpvals(-log(PVC(x, y, z,:)), dh) );
    if isnan(pTFCE(x,y,z))
        error('nan value')
    end
end
...
%     if isnan(dclust(h, V, Rd, c)) % underflow
%         dvc=dvox(h)*dclust(h, V, Rd, c);
%         return
%     end
...
function pvc=pvox_clust(V, Rd, c, actH, ZestThr) % p-value for Z threshold value given cluster size
    ...
    % ToDo: underflow handling
%     if isnan(dvox_clust(actH, V, Rd, c, ZestThr)) % underflow
%         pvc=exp(-745); %#TODO: make machnie independent
%         return 
%     end
