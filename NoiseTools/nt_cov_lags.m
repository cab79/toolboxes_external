function [C,tw,m]=nt_cov_lags(x,y,lags)
%[C,tw,m]=nt_cov_lags(x,y,lags) - covariance of [x,y] with lags
%
...
if isnumeric(x)
    ...
    n=size(x,2)+size(y,2);
    C=zeros(n,n,length(lags));
    for iPage=size(x,3)
        for iLag=1:length(lags)
            [xx,yy]=nt_relshift(x,y,lags(iLag));
    ...
    for iCell=1:length(x);
         C=C+nt_cov_lags(x{iCell},y{iCell},lags);
