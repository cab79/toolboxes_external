...
for nSub = 1:numSubjects
    ...
    EEG_all(nSub) = icatb_eeg_pop_loadset(currentErpFile); % load .set/.dat file
    [mark(nSub).conds, mark(nSub).tnums, mark(nSub).fnums, mark(nSub).bnums] = get_markers(EEG_all(nSub));
...
for nSub = 1:numSubjects
    ...
    % No. of events
    %events = (1:n_trials);
    events = mark(nSub).tnums;

    if isfield(EEG, 'event') && strcmp(sorting_trials,'yes')
